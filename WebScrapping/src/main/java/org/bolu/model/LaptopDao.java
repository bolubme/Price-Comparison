...
public class LaptopDao {
    ...
    /**
     * Finds a LaptopVariation entity in the database based on display, processor, storage, and memory.
     *
     * @param laptopVariation The LaptopVariation object used to search for a matching entity.
     * @return The found LaptopVariation object or null if not found.
     */
    public LaptopVariation findLaptopVariation(LaptopVariation laptopVariation) {
        Session session = sessionFactory.getCurrentSession();
        try {
            session.beginTransaction();

            String queryStr = "from LaptopVariation where display='" + laptopVariation.getDisplay() + "' AND processor='" + laptopVariation.getProcessor() + "' AND storage='" + laptopVariation.getStorage() + "' AND memory=" + laptopVariation.getMemory();

            List<LaptopVariation> laptopVariationList = session.createQuery(queryStr).getResultList();

            if (laptopVariationList.isEmpty()) {
                return null;
            } else {
                return laptopVariationList.get(0);
            }
        } catch (Exception e) {
            if (session.getTransaction() != null) {
                session.getTransaction().rollback();
    ...
    /**
     * Finds a Comparison entity in the database based on price URL.
     *
     * @param comparison The Comparison object used to search for a matching entity.
     * @return The found Comparison object or null if not found.
     */
    public Comparison findComparison(Comparison comparison) {
        Session session = sessionFactory.getCurrentSession();
        try {
            session.beginTransaction();

            String queryStr = "from Comparison where priceUrl='" + comparison.getPriceUrl() + "'";
